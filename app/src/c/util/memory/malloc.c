...
void *bmalloc(size_t size) {
  ...
  CLAWD_LOG(APP_LOG_LEVEL_DEBUG, "malloc request: %d; free: %d", size, heap_size);
  while (true) {
    heap_size = heap_bytes_free();
    if (heap_bytes_free() > 750) {
      void *ptr = malloc(size);
      if (ptr) {
        CLAWD_LOG(APP_LOG_LEVEL_DEBUG_VERBOSE, "malloc returned %p for caller %p", ptr, saved_lr);
        return ptr;
      }
      CLAWD_LOG(APP_LOG_LEVEL_WARNING, "Out of memory! Need to allocate %d bytes; %d bytes free.", size, heap_size);
    } else {
