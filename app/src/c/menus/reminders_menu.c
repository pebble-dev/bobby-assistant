...
static void prv_draw_row(GContext *ctx, const Layer *cell_layer, MenuIndex *cell_index, void *context) {
  ...
  if (reminder->time < now) {
    strftime(time_text, sizeof(time_text), "Overdue - %I:%M %p", reminder_time);
  } else if (reminder->time < now + 24*60*60) {
    strftime(time_text, sizeof(time_text), "Today %I:%M %p", reminder_time);
  } else {
    strftime(time_text, sizeof(time_text), "%b %d %I:%M %p", reminder_time);
  }
  ...
  graphics_draw_text(ctx, reminder->text,
    ...
    GRect(4, 0, bounds.size.w - 8, 24),
    GTextOverflowModeTrailingEllipsis,
    GTextAlignmentLeft,
    NULL);
  graphics_draw_text(ctx, time_text,
    fonts_get_system_font(FONT_KEY_GOTHIC_18),
    GRect(4, 24, bounds.size.w - 8, 18),
    GTextOverflowModeTrailingEllipsis,
    GTextAlignmentLeft,
    NULL);
}
